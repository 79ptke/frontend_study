...
function solution(rank, attendance) {
    const answer = rank.map((el, index) => [el, index]).filter((v, idx) => { return attendance[idx] === true}).slice().sort(function(a, b) {return a[0] - b[0];}).map((element) => element[1]);
...
solution();
```
sort 뒤에 굳이 map을 안쓰고 정답을 answer[0][1]*10000 + answer[1][1]*100 + answer[2][1];  
이런식으로도 쓸 수 있었는데 그게 싫어 map을 썼더니 좀 투머치한 것 같다. 굳이 안써도 되는 것을 한 번 더 쓴 느낌..?
다른 사람 풀이를 보고 더 그렇게 느꼈다.
...
```
이렇게 [a,b,c]를 선언해놓고 쓰는 방식도 있는데 이게 더 깔끔한듯하다.
